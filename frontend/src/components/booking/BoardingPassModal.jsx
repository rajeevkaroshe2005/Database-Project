...
export default function BoardingPassModal({ booking, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-space-950/85 backdrop-blur-md animate-in fade-in">
      <div className="relative w-full max-w-2xl glass-panel rounded-3xl border border-white/20 shadow-2xl p-6 space-y-6">
        ...
        <div className="relative rounded-3xl bg-gradient-to-r from-space-900 via-[#0d1424] to-space-900 border border-cyan-500/30 shadow-2xl overflow-hidden text-white flex flex-col md:flex-row print:bg-white print:text-black print:border-black">
          {/* Main Body Pass */}
          ...
          <div className="relative border-t md:border-t-0 md:border-l-2 border-dashed border-white/20 p-6 bg-space-950/60 flex flex-col items-center justify-center gap-3">
            <div className="p-2 rounded-xl bg-white shadow-lg">
              <QRCodeSVG value={booking.qr_code_token || booking.booking_ref} size={80} />
            </div>
            <div className="text-center">
              <span className="text-[10px] font-mono font-bold text-cyan-400 block">{booking.booking_ref}</span>
