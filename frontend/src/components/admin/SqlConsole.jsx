...
export default function SqlConsole() {
  ...
  const getAccessTypeBadge = (type) => {
    ...
    return { label: type || 'N/A', color: 'bg-white/10 text-slate-300 border-white/10', desc: 'Custom Scan' };
  };
      ...
      <div className="glass-panel p-6 rounded-3xl border border-purple-500/30 bg-gradient-to-r from-purple-950/30 via-space-900 to-cyan-950/30 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-lg font-black text-white">Interactive SQL Console & EXPLAIN Optimizer</h3>
        ...
        <div className="bg-space-900 px-4 py-2.5 border-b border-white/10 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="px-3.5 py-1.5 rounded-xl text-xs font-bold glass-panel border border-purple-500/40 text-purple-300 hover:text-white hover:bg-purple-500/20 flex items-center gap-1.5 transition-all shadow-glow-purple"
        ...
        <div className="glass-panel rounded-3xl border border-purple-500/30 overflow-hidden space-y-6 p-6 animate-in fade-in">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-white/10 text-xs">
            <div className="flex items-center gap-2">
              ...
              <div>
                <h4 className="text-sm font-extrabold text-white">MySQL Query Execution Plan (EXPLAIN Output)</h4>
                <p className="text-[11px] text-slate-400">Analysis of how MySQL Cost-Based Optimizer executes this query</p>
              </div>
            </div>
          ...
          <div className="overflow-x-auto rounded-2xl border border-white/10 bg-space-950/80">
                ...
                {explainResult.plan?.map((step, idx) => {
                  ...
                  return (
                    <tr key={idx} className="hover:bg-white/5 transition-colors">
                      <td className="p-3 font-bold text-white">{step.id}</td>
                      ...
                      <td className="p-3 text-slate-400">{step.possible_keys || 'NULL'}</td>
                      <td className="p-3 font-bold text-emerald-300">{step.key || 'None (Table Scan)'}</td>
                      <td className="p-3 text-slate-300">{step.key_len || 'NULL'}</td>
                      <td className="p-3 text-amber-300 font-bold">{step.rows}</td>
                      <td className="p-3 text-slate-300">{step.filtered ? `${step.filtered}%` : '100%'}</td>
                      <td className="p-3 font-sans text-slate-300 max-w-xs truncate" title={step.Extra}>
            ...
            <div className="glass-panel p-4 rounded-2xl border border-white/10 space-y-2">
              ...
              <div className="font-mono text-xs text-white">
                {explainResult.plan?.[0]?.key ? (
                  ...
                  <span className="text-slate-400">No B-Tree Index used</span>
                )}
              </div>
