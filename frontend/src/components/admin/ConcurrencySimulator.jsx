...
export default function ConcurrencySimulator() {
  ...
  return (
    <div className="space-y-6">
      {/* Header Banner */}
      <div className="glass-panel p-6 rounded-3xl border border-cyan-500/20 bg-gradient-to-r from-space-950 via-cyan-950/20 to-space-950 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
        ...
        <button
          ...
          disabled={running}
          className="glow-button px-6 py-3 rounded-2xl text-xs font-bold text-white flex items-center gap-2 shadow-glow-cyan shrink-0 self-start md:self-auto disabled:opacity-50"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="glass-panel p-5 rounded-2xl border border-white/10 space-y-4 flex flex-col justify-between">
          ...
          <div className="p-3.5 rounded-xl bg-space-950/80 border border-white/10 space-y-1.5 text-[11px] font-mono">
          ...
          <div className="glass-panel p-6 rounded-3xl border border-emerald-500/30 bg-gradient-to-r from-emerald-950/30 via-space-900 to-space-950 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-6 font-mono text-xs self-end md:self-auto">
              <div className="text-right">
                <div className="text-[10px] uppercase text-slate-400">Total Conflicts Blocked</div>
                <div className="text-lg font-black text-amber-400">{result.conflictsCount} / {result.totalRequests}</div>
              </div>
              <div className="text-right">
                <div className="text-[10px] uppercase text-slate-400">Total Run Latency</div>
                <div className="text-lg font-black text-cyan-400">{result.totalDurationMs} ms</div>
              </div>
            </div>
          </div>
          ...
          <div className="glass-panel rounded-3xl border border-white/15 overflow-hidden space-y-3 p-5">
            <div className="flex items-center justify-between pb-3 border-b border-white/10 text-xs">
              <div className="flex items-center gap-2">
                ...
                <h4 className="font-bold text-white">Live Competing Transaction Execution Log</h4>
              </div>
            ...
            <div className="overflow-x-auto rounded-xl border border-white/10 bg-space-950/80 max-h-80 overflow-y-auto">
              <table className="w-full text-left text-xs font-mono">
                <thead className="bg-space-900 text-slate-400 uppercase text-[10px] tracking-wider border-b border-white/10 sticky top-0">
                  <tr>
                ...
                <tbody className="divide-y divide-white/5 text-[11px]">
                  {result.requests?.map((req, idx) => {
                    ...
                    return (
                      <tr key={idx} className={isWinner ? "bg-emerald-500/10" : "hover:bg-white/5 transition-colors"}>
                        <td className="p-3 text-cyan-300 font-bold">{req.txId}</td>
                        <td className="p-3 text-slate-300">{req.user}</td>
                        <td className="p-3">
                        ...
                        <td className="p-3 text-slate-400">{req.latencyMs} ms</td>
